...
Ciphertext CKKS_Ciphertext::init_from_ct_and_state(
    ...
    CKKS_Ciphertext new_ct = CKKS_Ciphertext(
        ct.a, ct.b, ct.pt_shape, t_state, ct.pt_scale);
    return new_ct;
}
...
CKKS_Ciphertext::CKKS_Ciphertext(string& proto_str) : CKKS_Ciphertext() {
...
Ciphertext CKKS_Ciphertext::make_copy(
    ...
    return CKKS_Ciphertext(
        new_ct.a, new_ct.b, new_ct.pt_shape, new_ct.state.value(), pt_scale);
}
...
CKKS_Ciphertext _CKKS::init_ct(
    ...
    std::vector<double> pt_scale = t_pt_scale ? t_pt_scale.value() : std::vector<double>(1, context.params.pt_scale);
    return CKKS_Ciphertext(ct.a, ct.b, ct.pt_shape, ct.state.value(), pt_scale);
}
...
std::tuple<TTensor, TTensor> _CKKS::dec_and_get_error(
    ...
    TTensor pt_crt_scaled = t_eng::modsum(ct.b, sk_a, state.q_list(), false);
    TTensor pt_coefs_scaled = crt_utils::crt_to_coefs_q(
        context, state, pt_crt_scaled, -2, false);

    Num q = Num(state.mod_state.active_q);
    Num half_q = q / 2;
    std::vector<Num> temp_res = mod_utils::from_crt_tensor_to_bigint(
        state.mod_state, pt_coefs_scaled, q
    );
...
TTensor _CKKS::unpack_pt(
    ...
    std::vector<double> pt_scale = t_pt_scale ? t_pt_scale.value() : std::vector<double>(1, context.params.pt_scale);
    for (int i=0; i<pt_scale.size(); i++) {
        pt_packed = pt_packed / pt_scale[i];
    }
    TTensor temp = t_eng::rfft(pt_packed.to(T_ENG_FLOAT_TYPE), context.params.n*2, -1);
    temp = temp.index({"...", Slice(1, context.params.n, 2)});
...
TTensor _CKKS::dec_and_unpack(
    ...
    pt = unpack_pt(context, pt, dynamic_cast<CKKS_Ciphertext*>(&ct)->pt_scale);
    if (convert_to_external) {
        pt = pt_shape::convert_internal_to_external(
            pt, dynamic_cast<CKKS_Ciphertext*>(&ct)->pt_shape);
    }
    return pt;
}
