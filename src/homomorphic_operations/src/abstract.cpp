...
size_t QueryClientBlock::get_op_types_size() const {
    return op_types.size();
}

lattica_proto::HomOpType& QueryClientBlock::get_op_type(size_t index) {
    if (index < op_types.size()) {
        return op_types[index];
    } else {
        throw std::out_of_range("Index out of range");
    }
}

lattica_proto::ApplyClearParams& QueryClientBlock::get_apply_clear_params(size_t index) {
    if (index < apply_clear_params_list.size()) {
        return apply_clear_params_list[index];
    } else {
        throw std::out_of_range("Index out of range");
    }
}
...
QueryClientBlock::QueryClientBlock(const std::string& proto_str)
    : QueryClientBlock() {
...
std::vector<lattica_proto::AuxKeyType>& QueryClientSequentialHomOp::get_key_types() {
    return key_types;
}
...
QueryClientSequentialHomOp::QueryClientSequentialHomOp(const std::string& proto_str)
