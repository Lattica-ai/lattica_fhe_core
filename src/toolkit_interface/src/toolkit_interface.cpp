...
std::tuple<std::tuple<string, string>, string> raw_generate_key(
    string& serialized_homseq,
    string& serialized_context) 
{
    ...
    if (context.is_ckks) {
        key_aux_proto_str = key_switch::KeyAux<encryption_schemes::_CKKS_G_CRT>().init_values(
            context, sk, client_seq_hom_op.key_types).to_proto().SerializeAsString();
    } else {
    ...
    optional<int> n_axis_external) 
{
    ...
    auto internal_n = context.params.n;
    if (context.is_ckks) {
        internal_n /= 2;
    }
    ...
    string& serialized_ct) 
{
    ...
    if (context.is_ckks) {
        ...
        ct.init(ct_proto);
        if (!ct.state.has_value()) {
            ct.state = context.init_state;
        }
    ...
    string& serialized_pt) 
{
    ...
    auto len_block = block.op_types.size();
    for (int i = 0; i < len_block; i++) {
        auto op_type = block.op_types[i];
        auto apply_clear_params = block.apply_clear_params_list[i];
